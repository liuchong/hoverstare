...
- `@hoverstare merge`——checks 全绿且无冲突后，它会 squash 合并并删除源分支。
...
- 推送请使用 `gh_pat` 输入传入 PAT，或具备 `contents: write` 的 GitHub App token——默认 `GITHUB_TOKEN` 的推送**不会**触发 CI，bot 提交将永远跑不了必需检查。合并同样需要 `contents: write`（squash 合并会在 base 分支产生提交）。
