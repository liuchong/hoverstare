...
- Для пушей передайте PAT через вход `gh_pat` или используйте токен GitHub App с `contents: write` — пуши с обычным `GITHUB_TOKEN` **не** запускают CI, и обязательные проверки на коммитах бота никогда не выполнятся. Для merge тоже нужен `contents: write` (squash-merge создаёт коммит в base-ветке).
