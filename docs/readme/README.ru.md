...
HoverStare умеет и *разрабатывать* — issues и PR превращаются в среду разработки, управляемую диалогом (spec 11):
...
- `@hoverstare merge` — когда проверки зелёные и нет конфликтов, он сделает squash-merge.

Настройка: добавьте триггеры `issues` и `pull_request_review` и выдайте `contents: write` + `issues: write`. Полный рабочий пример — `.github/workflows/hoverstare.yml`. Примечания:

- Команды может отдавать только коллаборатор репозитория; fork PR не поддерживаются.
- Для пушей передайте PAT через вход `gh_pat` или используйте токен GitHub App с `contents: write` — пуши с обычным `GITHUB_TOKEN` **не** запускают CI, и обязательные проверки на коммитах бота никогда не выполнятся.
