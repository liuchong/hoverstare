...
| **空队列不自触发**：链在队列排空时结束 | 最后一轮落地后 PR 上不再出现新的 `@hoverstare continue` 评论。单测：`devqueue::tests::self_trigger_only_after_landed_round_with_work_left` |
...
| **本轮条目 Running → 结束置 Done/Failed（失败即停）** | 轮次报告的队列状态行先显 `running`，收尾按结果落 `done` / `failed`，`failed` 不自动重试。单测：`devqueue::tests::self_trigger_only_after_landed_round_with_work_left` |
