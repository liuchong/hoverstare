...
> 本文档记录 bugbot 在 GitHub 真实环境的端到端验证过程与结果。
> 验证仓库：github.com/liuchong/bugbot（PR #1 为演示 PR）。
...
| 摘要 + Nitpicks + 元数据 | ✅ | body 含变更概述、阈值下低级别发现（u64 溢出）、bugbot-meta 注释 |
...
| resolve 线程 | ✅（降级路径） | 默认 GITHUB_TOKEN 受限 → 自动降级为线程内回复"✅ Bugbot 已确认修复"×3 |
| `@bugbot explain` | ✅ | 线程内回复结构化解释（是什么/何时触发/影响/怎么修）+ 🚀/+1 reactions |
| `@bugbot help` | ✅ | 命令列表回复 |
...
3. **workflow 缺 pull_request_review_comment 触发**：`@bugbot explain` 的线程回复
