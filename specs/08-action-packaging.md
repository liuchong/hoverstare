...
1. `cargo build --release --target x86_64-unknown-linux-musl`，`strip`；
2. 产物打包 + `sha256sum`；
3. `softprops/action-gh-release` 创建 Release 并上传产物；
4. 大版本 tag（`v1`）force-move 到最新 `v1.x.y`（用户侧 `@v1` 自动跟进修复版）。
