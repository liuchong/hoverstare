...
Bugbot est un bot de revue de code IA pour les pull requests GitHub, écrit en
...
- 🔁 **Revues incrémentales.** Poussez un correctif et Bugbot ne revoit que le
...
`chemin + ligne de code + titre`). Au prochain push, Bugbot compare avec sa
...
**1. Ajoutez le workflow** — `.github/workflows/bugbot.yml` :
    ...
    steps:
      ...
      - uses: liuchong/bugbot@v0
        env:
          ...
          BUGBOT_MODEL: ${{ vars.BUGBOT_MODEL }}   # ex. kimi-for-coding
```
...
| **Compatible OpenAI** (Kimi, DeepSeek, OpenRouter…) | secret `OPENAI_API_KEY`, variable `OPENAI_BASE_URL` (ex. `https://api.kimi.com/coding/v1`), nom du modèle via `BUGBOT_MODEL` ou `model` dans `.github/bugbot.toml` |

> ⚠️ Avec un endpoint compatible OpenAI, vous **devez** définir le nom du
> modèle — le défaut `claude-sonnet-4-6` n'existe pas là-bas.

**3. (Optionnel) Config du dépôt** — `.github/bugbot.toml`, tous champs optionnels :
...
status_checks = false                 # écrire les checks bugbot / bugbot-findings
...
| Commande | Effet |
|---|---|
| `@bugbot review` | Force une re-revue complète |
| `@bugbot explain` | Répond dans le fil avec une explication en langage clair |
| `@bugbot help` | Liste des commandes |
